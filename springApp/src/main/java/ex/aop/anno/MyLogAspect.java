...
/*
 - @Aspect
 	: 설정 파일에서 Advice 및 Pointcut 등의 설정을 하지 않고 자동으로 Advice 적용
 */

@Aspect
public class MyLogAspect {
	@Pointcut(value = "execution(public * ex.aop.anno.core..*Service.*(..))")
	private void publicMethod() {
		// Pointcut 설정을 위한 가명 메소드 
		// private void 이어야 하며, 메소드의 몸체가 없어야 함
	}
	
	// 메소드 호출전 
	//@Before("execution(public * ex.aop.anno.core..*Service.*(..))")
	@Before("publicMethod()")
	...
	public void returningLogging(JoinPoint jp, Object ret) {
		...
		System.out.println("returning : " + methodName + 
				", 리턴값 : " + ret);
	}
